...
export function AppLayout() {
  ...
  // Менеджеры работают в режиме аналитики; загрузку данных делает только
  // администратор — скрываем admin-only пункты меню для менеджера.
            ...
            return (
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon
                      className={cn('h-[18px] w-[18px]', isActive ? 'text-accent-lime' : '')}
                      strokeWidth={1.75}
                    />
                    <span className="flex flex-col">
                      <span className="font-medium">{entry.label}</span>
                      <span className="text-[11px] text-text-muted">{entry.hint}</span>
                    </span>
                  </>
                )}
              </NavLink>
