/* Кастомный узел-показатель для React Flow.
   Сигнатурный эффект: лёгкий 3D-наклон карточки по курсору (≤6°),
   единственная анимация узла. Отключается при prefers-reduced-motion. */
...
/** Максимальная амплитуда наклона, градусы (держим в пределах 4–6°). */
const TILT = 5
...
function IndicatorNodeImpl({ data, selected }: NodeProps) {
  ...
  // Нормализованная позиция курсора над карточкой: -0.5 … 0.5.
  ...
  return (
    <motion.div
      ...
      // Фиксируем геометрию, чтобы совпадала с dagre-лэйаутом.
      // eslint-disable-next-line react/forbid-dom-props
      // (ширина/высота задаются инлайн осознанно — это размеры узла)
