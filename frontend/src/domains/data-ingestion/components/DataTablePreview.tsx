...
import { generateLargeDataset } from '@/mocks/largeDataset'
import { formatNumber, formatPeriod } from '@/shared/lib/format'

/* ============================================================
   Виртуализированная таблица предпросмотра (раздел 3.2).
   150 000 строк, но в DOM рендерятся только видимые ~20-30:
   - сортировка и фильтрация выполняются НА УРОВНЕ ДАННЫХ (массив),
     а не через DOM или встроенные row-models react-table;
   - useVirtualizer рендерит окно строк + overscan.
   ============================================================ */

const ROW_HEIGHT = 36 // px
const CONTAINER_HEIGHT = 480 // px фиксированной высоты со внутренним скроллом
const DATASET_SIZE = 150_000
...
export function DataTablePreview() {
  // Датасет генерируется один раз — тяжёлый массив не пересоздаём на ререндере.
  const allRows = useMemo<FactRow[]>(() => generateLargeDataset(DATASET_SIZE), [])

  const [query, setQuery] = useState('')
  const [sortKey, setSortKey] = useState<SortKey | null>(null)
  const [sortDir, setSortDir] = useState<SortDir>('asc')

  // Фильтрация НА УРОВНЕ ДАННЫХ по subject/indicator.
  ...
  // Сортировка НА УРОВНЕ ДАННЫХ (по сырому value, не по отформатированной строке).
  ...
  // react-table используем как лёгкий каркас; row-model — core, без сортировки/фильтра внутри.
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="tabular text-sm text-text-secondary">
          ...
          {rows.length !== allRows.length && (
            <span className="text-text-muted"> из {formatNumber(allRows.length)}</span>
          )}
        </span>
      </div>

      <div className="overflow-hidden rounded-card border border-border">
        {/* Внутренний скролл-контейнер фиксированной высоты. */}
        <div
          ref={scrollRef}
