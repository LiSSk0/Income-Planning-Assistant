...
/* Блок «Прогноз по показателям». План прогнозирует ВСЕ доступные показатели
   (scenario.seriesByIndicator). Чекбоксы выбирают, какие показатели показывать
   на графике (динамика, индекс к началу = 100) и на диаграмме (прирост за
   период, %). И график, и диаграмма строятся по выбранным чекбоксами показателям. */

interface Props {
  scenarios: Scenario[]
}

/** Короткие подписи показателей для осей/диаграммы. */
...
/** Палитра по показателям (нужны конкретные цвета для SVG). */
const PALETTE = [
  chartTheme.lime,
  chartTheme.green,
  chartTheme.blue,
...
export function PlanIndicatorsView({ scenarios }: Props) {
  ...
  // Планы с прогнозом по показателям.
  const ready = useMemo(
    () =>
      scenarios.filter(
  ...
  // Цвет закреплён за показателем по его позиции в полном списке.
  ...
  // Выбранные чекбоксами показатели (по умолчанию — все).
  ...
  // Данные графика: индекс к первому значению периода (база = 100),
  // чтобы показатели с разными единицами были сопоставимы на одной оси.
  ...
  // Данные диаграммы: прирост показателя за период, % (конец vs начало).
